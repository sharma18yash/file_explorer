...
string getFileCreationTime(char *path) 
{
    ...
    if(attr.st_gid=1000)
        ls+="yash\t";
    ls+=to_string(attr.st_size/1024)+"KB\t";
...
void commandMode(struct abuf *ab)
{
  ...
  while(1)
  {
    ...
    {
      string path;
      int lastI = commandVec.size()-1;
      if(commandVec[lastI][0]=='~' && commandVec[lastI][1]=='/')
      {
        commandVec[lastI].erase(0,2);
        path = ROOT+commandVec[lastI];
        commandVec[lastI] = path; 
      }
      for(int i=1; i<commandVec.size()-1; i++)
      {
        string source_dir = cwds + '/' + commandVec[i];
        string dest_dir =  commandVec[lastI] + "/" + commandVec[i];
